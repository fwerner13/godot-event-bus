...
func publish(event_name, value = null):
	if (!receivers.has(event_name)):
		push_warning("event '" + event_name + "' has no receivers")
		return
	...
	for receiver in event_receivers.keys():
		...
		if (null == receiver): 
			# should we automatically unsubscribe the receiver if it's not reachable?
			push_warning("event " + event_name + " receiver not reachable. Unsubscribe before freeing a receiver.")
			continue
